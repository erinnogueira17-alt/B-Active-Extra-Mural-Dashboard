...
function tryReadHeader(row) {
  ...
  const sponsoredIdx = row.findIndex((c, idx) => idx < payingIdx && (c || "").trim() === "100%");
  return { paying: payingIdx, enrolled: payingIdx - 1, revenue: revenueIdx, sponsored: sponsoredIdx };
}
...
export function aggregateCurrentState(parseResult, { now = new Date() } = {}) {
  ...
  const totals = rows.reduce(
    ...
    { payingPlayers: 0, enrolledPlayers: 0, sponsoredPlayers: 0, revenue: 0 }
  );
