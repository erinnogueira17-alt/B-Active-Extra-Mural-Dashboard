...
// against the roster. Parenthetical groups carry day/time/grade info in
// some rows and nothing meaningful in others ("(Register)", "(SOE)"
// prefixes) — stripped wholesale rather than pattern-matched, since the
// school name itself is never inside parens on either side.
...
// of that school's distinctive name tokens also appear in the venue text.
// Requires at least 2 overlapping tokens (or the single token, for a rare
// one-word school name) so a couple of generic shared words ("pre",
// "school") can't produce a false match on their own.
function matchSchool(rawVenue, index) {
  ...
  for (const entry of index) {
    ...
    const required = entry.tokens.size === 1 ? 1 : Math.max(2, Math.ceil(entry.tokens.size * 0.6));
