  ...
  return headers
    .map((key) => {
      ...
      for (const raw of rawValues) {
        ...
        if (d && d.getUTCFullYear() > 2015 && d.getUTCFullYear() < 2035) valid++;
      }
      const checked = rawValues.length;
      return { key, checked, valid, score: checked ? valid / checked : 0, dayFirst };
    })
...
export function detectDateColumn(rows, { sampleSize = 200 } = {}) {
  ...
  if (best && best.score >= 0.6) return { key: best.key, dayFirst: best.dayFirst };
