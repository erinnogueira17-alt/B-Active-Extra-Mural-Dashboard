...
// Detects the submission-timestamp column by content, not by header name.
// These sheets are heavily hand-edited across years, and the live-year tab
// sometimes lacks a "Timestamp" header at all (renamed, blank, or the real
// date column has shifted) even though a genuine, mostly-valid date column
// still exists somewhere in the row. Scanning every column for which one is
// consistently parseable as a plausible date is far more robust than
// trusting any particular header label to still be there.
function guessTimestampKey(rows) {
  ...
  // Require most sampled values in the winning column to actually look like
  // dates, not just the best of a bad lot.
  if (bestKey && bestScore >= 0.6) return bestKey;

  return findHeaderKey(headers, "timestamp") || headers[0] || null;
}

function datesOf(rows) {
  const key = guessTimestampKey(rows);
  if (!key) return [];
  return rows.map((r) => parseTimestamp(r[key])).filter(Boolean);
}
