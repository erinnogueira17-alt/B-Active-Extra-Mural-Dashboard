...
export function aggregateGrowth(datasets, schoolRegionIndex, { now = new Date() } = {}) {
  ...
  // daily/weekly series, region split, trial outcomes, B-less reasons,
  // package mix, top venues) is automatically football-free rather than
  // each one needing its own exclusion logic.
  ...
  return {
    ...
    blessReasons: tallyField(filteredDatasets.bless, "__reason", { multiSelect: true, limit: 12 }),
