...
// B-Active tracks Johannesburg extramural, Cape Town extramural, and
// Football as three separate departments even though all three share the
// same intake forms. Splits each metric's season total (same window as
// the headline "Season totals" figure, so the split sums back to it) by
// classifying each row's venue against the Current State roster via
// classifyVenue. schoolRegionIndex is built once per sync from that
// roster (see lib/venueRegion.js) and passed in rather than rebuilt here.
function regionTotalsOf(datasets, schoolRegionIndex, monthWindows) {
  ...
  const totals = {
    ...
    football: { intentions: 0, enrolments: 0, bless: 0 },
...
export function aggregateGrowth(datasets, schoolRegionIndex, { now = new Date() } = {}) {
  ...
  return {
    ...
    daily,
    weekly,
    regionTotals: regionTotalsOf(datasets, schoolRegionIndex, monthWindows),
    trialOutcomes: tallyField(datasets.intentions, "__status"),
    blessReasons: tallyField(datasets.bless, "__reason", { multiSelect: true }),
    packageMix: tallyField(datasets.enrolments, "__package"),
    topVenues: {
      intentions: tallyField(datasets.intentions, "__venue", { limit: 10 }),
      enrolments: tallyField(datasets.enrolments, "__venue", { limit: 10 }),
      bless: tallyField(datasets.bless, "__venue", { limit: 10 }),
    },
    compare: {
