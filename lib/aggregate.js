...
// Season window is hardcoded per project decision: Jan 2026 - Dec 2026 (12
// months, a full calendar year). Extending into a second season requires
// updating this function.
export function buildWindowMonths() {
  const months = [];
  let year = 2026;
  let month = 0; // January, 0-based
  for (let i = 0; i < 12; i++) {
    months.push({
...
// Surfaces exactly which raw venue text is landing in "unclassified" — the
// actual school/venue strings classifyVenue couldn't confidently match to a
// roster school — so the business can see, directly from the dashboard,
// which real venues need fixing (usually: the roster's school name and the
// form's venue dropdown text have drifted too far apart for matchSchool's
// token-overlap bar to clear) instead of only via a one-off ?debug=1 sync
// call. Tallied over the exact same rows/window regionTotalsOf uses, so the
// counts always agree with what the "Unclassified" region card shows.
function unclassifiedVenuesOf(datasets, schoolRegionIndex, monthWindows) {
  const windowKeys = new Set(monthWindows.map((m) => m.key));
  const counts = new Map();
  for (const metric of ["intentions", "enrolments", "bless"]) {
    for (const row of datasets[metric] || []) {
      const date = parseTimestamp(row.__timestamp);
      if (!date || !windowKeys.has(monthKeyOf(date))) continue;
      if (classifyVenue(row.__venue, schoolRegionIndex) !== "unclassified") continue;
      const venue = (row.__venue || "").trim() || "(blank)";
...
// Enrolments only. A staff member enrolling their own child isn't a real
// paying customer — per business direction, exclude these from every
// Enrolments count in reporting. Identified by the Player Enrolment form's
// "Payment option" column: a staff enrolment reads "Staff" there instead of
// a real payment method (EFT, Debit Order, Cash, ...). Matched by
// case-insensitive substring, same approach as isEndingMembership/
// isPauseOnly above, since the exact real cell text (e.g. "Staff" vs "Staff
// Member" vs "Staff - No Charge") wasn't confirmed ahead of this change.
function isStaffEnrolment(row) {
  return /staff/i.test(row.__paymentOption || "");
}

// B-less only. The "What would you like to do with your membership?" field
// on that form is a checkbox (values can be a comma-separated combination,
// e.g. "End my membership, Pause Account"), with real options confirmed
// directly against the sheet: "End my membership", "Pause Account", "Move
// to another venue/school if possible", "End only one service", and
// combinations of these. A pause, a venue move, or "end only one service"
// is not a B-less by the business's own definition — only an actual
// membership ending is a real lost player — so this keeps a row only when
// "End my membership" is selected AND "Pause Account" is not (per business
// direction: any row mentioning a pause is excluded from B-less outright,
// even combined with "End my membership" — it reads as ambiguous intent,
// not a confirmed loss, so it's treated as a pause, not a B-less).
function isEndingMembership(row) {
  const action = row.__membershipAction || "";
  return /end my membership/i.test(action) && !/pause account/i.test(action);
}

// Any row selecting "Pause Account", in any combination — mutually
// exclusive with isEndingMembership by construction (see above), so every
// row lands in exactly one of "real B-less" or "paused", never both.
function isPauseOnly(row) {
  return /pause account/i.test(row.__membershipAction || "");
}

// One entry per player currently paused. The B-less form has no separate
// "resumed" signal, so "currently paused" here means: this player's most
// recent B-less-form submission (by real timestamp) selected Pause Account.
// If they later submit again with "End my membership" and no pause, that
// later row is a real B-less (isEndingMembership) and they drop off this
// list entirely — but if someone simply returns to sessions without ever
// resubmitting the form, this can't know that on its own and needs a
// person to confirm it. Deduplicated by player name, case- and
// whitespace-insensitively, so the same person spelled slightly differently
// across two submissions doesn't produce two rows.
function pausedPlayersOf(rows, schoolRegionIndex) {
  const latestByName = new Map();
  for (const row of rows) {
    const name = (row.__playerName || "").trim();
    if (!name) continue;
    const date = parseTimestamp(row.__timestamp);
    if (!date) continue;
    const dedupeKey = name.toLowerCase();
    const existing = latestByName.get(dedupeKey);
    if (!existing || date > existing.date) {
      latestByName.set(dedupeKey, { name, row, date });
    }
  }

  const paused = [];
  for (const { name, row, date } of latestByName.values()) {
    if (!isPauseOnly(row)) continue;
    paused.push({
      name,
      venue: row.__venue || "",
      region: classifyVenue(row.__venue, schoolRegionIndex),
      reason: row.__reason || "",
      pausedAt: date.toISOString(),
    });
  }
  return paused.sort((a, b) => (a.pausedAt < b.pausedAt ? 1 : -1));
}
...
export function aggregateGrowth(datasets, schoolRegionIndex, { now = new Date() } = {}) {
  ...
  // its own exclusion logic. blessAllRows keeps every membership-action
  // type (needed for pausedPlayersOf below); filteredDatasets.bless narrows
  // that down further to only real membership-ending rows (see
  // isEndingMembership above) — a paused or moved membership isn't a B-less.
  const blessAllRows = excludeFootball(datasets.bless, schoolRegionIndex);
  const filteredDatasets = {
    intentions: excludeFootball(datasets.intentions, schoolRegionIndex),
    enrolments: excludeFootball(datasets.enrolments, schoolRegionIndex).filter(
      (r) => !isStaffEnrolment(r)
    ),
    bless: blessAllRows.filter(isEndingMembership),
  };
  ...
  // "Month to date" here means the same month the rest of the dashboard
  // already treats as current — the most recent month in `months` that has
  // actually started (buildMonthlySeries leaves every future month `null`).
  // Reusing regionTotalsOf/unclassifiedVenuesOf with just that one month's
  // window, instead of the full season's, gives a same-shape result for
  // free with no separate month-filtering logic to keep in sync.
  const monthToDateEntry = [...months]
    .reverse()
    .find((m) => m.intentions != null || m.enrolments != null || m.bless != null);
  const monthToDateWindow = monthToDateEntry
    ? monthWindows.find((w) => w.key === monthToDateEntry.key)
    : null;
  const monthToDateWindows = monthToDateWindow ? [monthToDateWindow] : [];
  ...
  return {
    ...
    regionTotalsMonthToDate: monthToDateWindow
      ? regionTotalsOf(filteredDatasets, schoolRegionIndex, monthToDateWindows)
      : null,
    unclassifiedVenues: {
      season: unclassifiedVenuesOf(filteredDatasets, schoolRegionIndex, monthWindows),
      monthToDate: unclassifiedVenuesOf(filteredDatasets, schoolRegionIndex, monthToDateWindows),
    },
    ...
    pausedPlayers: pausedPlayersOf(blessAllRows, schoolRegionIndex),
    compare: {
