...
// membership ending is — so this keeps a row only when "End my membership"
// is one of the selected options, combos included.
function isEndingMembership(row) {
  return /end my membership/i.test(row.__membershipAction || "");
}

// A row selecting "Pause Account" (alone or combined with something other
// than "End my membership") — real options and combos confirmed directly
// against the sheet, same field as isEndingMembership above. An ended
// membership always wins over a pause on the same row (matches the one
// real combo seen in the data, "End my membership, Pause Account") — that
// player left, they're not merely paused.
function isPauseOnly(row) {
  return /pause account/i.test(row.__membershipAction || "") && !isEndingMembership(row);
}
...
// without also ending their membership. If they later submit again with
// "End my membership", that later row wins (isEndingMembership) and they
// drop off this list entirely — but if someone simply returns to sessions
// without ever resubmitting the form, this can't know that on its own and
// needs a person to confirm it. Deduplicated by player name, case- and
